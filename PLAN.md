...
## 15. v1.1 — Bug fixes & upgrades (2026-06-05)

Applied on top of v1. **Level definitions are unchanged**: range = yesterday's daily high/low (00:00 UTC boundary); swing = nearest *qualifying* pivot beyond the range, scanning back through all available daily history. These changes fix marking precision, alert noise, and observability — verified against the running app on HYPE.

### 15.1 Swing minimum-distance rule (fixes fake-precision swings) — DONE
**Problem:** with HYPE at its all-time high, the app marked `swingHigh = 75.825` against `rangeHigh = 75.5` — only **0.43% above**, i.e. the same peak, not a separate level. Same fake-precision seen on DOGE (`swingLow` 0.3% below `rangeLow`).
**Fix:** a pivot only qualifies as a swing if it is at least `config.swingMinDistancePct` (**default 0.015 = 1.5%**) beyond the range:
- `swingHigh` qualifies iff `high >= rangeHigh * (1 + swingMinDistancePct)`
- `swingLow` qualifies iff `low <= rangeLow * (1 - swingMinDistancePct)`
Keep scanning back; if none qualifies in all history, the swing is `null` and is not drawn (the dashboard already hides null swings). Implemented in `core/levels.ts`; covered by a unit test in `core.test.ts`.
**Verify:** HYPE `swingHigh = null`; `swingLow` (56.25, ~10.7% below) stays.

### 15.2 Signal feed: "stale rangeLow" was a misread — real causes fixed — DONE
**Investigated against the DB.** The detection engine was **not** using stale levels. The feed showed many `LEVEL_BREAK — rangeLow 68.274` entries while the chart showed `rangeLow 63`, but:
- 68.274 was the **06-03** daily low — the genuine range during calendar day **06-04**, when those events fired.
- 63 is the **06-04** daily low — the range for **06-05**; today's newest events correctly reference 63.
The engine reads the same recomputed `Levels` the chart/panel render (single source of truth: the in-memory `activeLevels`, written together with the DB row on every compute), and levels recompute both on the 00:00-UTC rollover **and** whenever a `1d` candle closes. The misleading impression came from two real issues, now fixed:
1. **`LEVEL_BREAK` had no cooldown** — a sustained breakdown fired a break on *every* 15m candle, flooding the feed with identical entries. Fix: apply the same `touchCooldownMinutes` dedup to `LEVEL_BREAK` that `LEVEL_TOUCH` already had (`core/detect.ts`, generalised `recentEventWithinCooldown`).
2. **Per-candle engine logging** — the monitor now logs `[eval] <coin> <ts> close=… | rangeHigh=… rangeLow=… swingHigh=… swingLow=…` for every detection candle, so each signal is traceable to the exact levels used and engine/chart agreement is visible.
**Note for the dashboard (web lane):** the signal feed should also show the **date** (not just HH:MM) on each row so prior-day events are distinguishable.
**Verify:** newest HYPE signals reference 63; repeated identical breaks are suppressed within the cooldown.

### 15.3 Socket reconnect — confirmed working, no change
The "Socket Offline" state was because no monitor process was running, not a code fault. The reconnect-with-backoff and the `staleSocketSeconds` (90s) watchdog that force-reconnects a silently dead socket are present in `monitor/hyperliquid.ts` and verified.

### 15.4 New config
- `swingMinDistancePct: 0.015` — minimum distance a swing must sit beyond the range.
...
# v3 — Historical per-day levels overlay (toggle)

**Status: SPEC ONLY — not implemented. For Codex to build.** Written 2026-06-05.

## v3.1 Goal

Add a **toggle on the chart** ("History"). When ON, overlay the **historical computed levels for every day currently loaded on the chart** — each day's **rangeHigh, rangeLow, swingHigh, swingLow** — drawn as **horizontal segments bounded to that day's width**, in the existing four colours. The result is a stair-stepped map of how the levels moved day to day (matches the user's manual 1h markup, where each day's prior-day high/low is a short horizontal line over that day's candles).

When OFF (default): exactly today's behaviour — only the current day's four levels, drawn full-width.

## v3.2 Scope

**In:** history toggle; per-day level computation across the loaded date range; day-bounded segmented rendering; the four existing colours.
**Out:** any change to level *definitions* (unchanged — §5.2 range + §15.1 swing min-distance + `swingLookbackDays: 0`); detection/alerts; new timeframes; new storage tables.

The range of history shown = **everything currently loaded on the chart** for the selected interval (15m ≈ 52d, 1h ≈ 208d, 4h, 1d per v2 backfill targets). Do **not** draw levels for days with no candles loaded.

## v3.3 Data — historical levels endpoint

Reuse the **pure** `computeLevels(dailyCandles, { now, swingLookbackDays, pivotWindow, swingMinDistancePct })` unchanged — it already resolves "yesterday relative to `now`". Compute one `Levels` per historical UTC day by calling it with `now = that day's 00:00 UTC`.

1. **core** (or monitor helper): add a thin pure helper, e.g. `computeLevelsRange(dailyCandles, fromDay, toDay, opts): Levels[]` — loops UTC days in `[fromDay, toDay]`, calls `computeLevels` with `now = dayStart`, skips days lacking a completed prior daily candle. Must use the **same** `swingLookbackDays` (0 = all history) and `swingMinDistancePct` as the live levels so historical and current values are computed identically.
2. **monitor/api.ts**: add read-only `GET /api/levels/history?coin=&from=&to=` returning `Levels[]` (each includes its `forUtcDay`). `from`/`to` are ms epoch; validate the coin against `status.coins`; clamp the range; if absent, default to the full cached daily range. Compute from the **cached `1d` candles** already in the store (`getRecentCandles(coin, '1d', …)`). No new storage — this is derived data. An in-memory cache keyed by `(coin, day)` is optional if perf needs it.

## v3.4 Rendering (`web/components/Chart.tsx`)

**Key constraint:** `createPriceLine` always spans the full chart width and **cannot** be time-bounded. Day-bounded segments require **line series** (`addLineSeries`), not price lines.

- Create exactly **four line series** — one per level type: rangeHigh (solid `#b94040`), swingHigh (dashed `#b94040`), rangeLow (solid `#20885f`), swingLow (dashed `#20885f`). Same colours/styles as today's price lines.
- For each historical day `D`, add a horizontal segment at that day's level value spanning `D`'s candle range:
  - points `{ time: dayStartBar, value }` and `{ time: dayEndBar, value }`, then a **whitespace** point `{ time: dayEndBar + 1 bar }` (no `value`) to break the line so days don't connect.
  - points per series sorted ascending by time; a `null` swing for a day → no segment that day (just the gap).
- The day a level is **active** is day `D`; its value derives from `D-1` (per §5.2). Draw the segment **over day `D`'s candles** (the day you'd trade that level), not the day the high/low formed.
- Series options: `lastValueVisible: false`, `priceLineVisible: false`, `crosshairMarkerVisible: false` — so the historical segments don't clutter the price axis or crosshair.
- Segment times must **align to the selected interval's bar times** (round `dayStart`/`dayEnd` to containing bars) or lightweight-charts may drop/misplace points. Reuse the interval→ms logic.

## v3.5 Frontend toggle (`web/app/page.tsx` + `Chart.tsx`)

- Add a **"History" toggle** near the timeframe selector (same styling as `TimeframeSelector`). Persisting to localStorage is optional.
- **OFF (default):** current behaviour — today's four levels as full-width `createPriceLine`; no history series.
- **ON:** keep today's full-width lines (today still stands out) **and** draw the four historical segment series for the loaded window. Fetch `/api/levels/history` when toggled on, and refetch when **coin or interval** changes, for `from = first loaded candle openTime`, `to = last loaded candle closeTime`.
- Pass history data + toggle state to `Chart` as props. Theme switching must keep the segments visible.
- **web/lib/api.ts:** add `getLevelsHistory(coin, from, to): Promise<Levels[]>`.

## v3.6 Build order

1. core: add `computeLevelsRange` (pure; reuses `computeLevels`).
2. monitor/api.ts: add `/api/levels/history` using cached `1d` candles + the helper.
3. web/lib/api.ts: add `getLevelsHistory`.
4. Chart.tsx: add the four segment line series + a `showHistory` prop; keep today's price lines.
5. page.tsx: add the History toggle, fetch history for the loaded range, wire props.

## v3.7 Acceptance criteria

- [ ] Toggle OFF = exactly current behaviour (today's four full-width lines only).
- [ ] Toggle ON on a 1h chart: each past day shows up to four short horizontal segments at that day's range/swing levels, in the four distinct colours, spanning only that day, stair-stepping with the market.
- [ ] Segments only appear for days with candles loaded — none floating in empty space; the set scales with the timeframe.
- [ ] A `null` swing for a day → that segment absent that day; range segments always present.
- [ ] Switching coin/interval refetches and redraws correctly; switching theme keeps segments visible.
- [ ] No price-axis label clutter from the historical segments.
- [ ] Historical level values exactly match what the live engine would have computed for that day (same `swingLookbackDays`/`swingMinDistancePct`).
- [ ] Detection/alerts unchanged.

## v3.8 Gotchas / verify-before-coding

- **`createPriceLine` is full-width only** → must use `addLineSeries` for day-bounded segments.
- **Line series need whitespace points between days** or every segment connects into one zig-zag line.
- **Segment times must align to the selected interval's bar times** (containing-bar rounding) or points get dropped/misplaced.
- **`computeLevels` resolves "yesterday" from `now`** — pass `now = D 00:00 UTC` to get day `D`'s active levels (range from `D-1`).
